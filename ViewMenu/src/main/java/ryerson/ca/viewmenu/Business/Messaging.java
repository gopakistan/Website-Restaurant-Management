...
import ryerson.ca.viewmenu.endpoint.MyAppServletContextListener;
            ...
            public void onNext(EventReceive value) {
                try {
                    ...
                    if(msgParts.length==3){
                        if(msgParts[0].equals("VIEW")){
                            int FoodID = Integer.valueOf(msgParts[1]);
                            int viewCount = Integer.valueOf(msgParts[2]);
                            
                            FoodItem_CRUD.updateViewCount(FoodID, viewCount);
                        }
                    }
